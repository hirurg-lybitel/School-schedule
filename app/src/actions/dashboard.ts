...
export function addDashboard(newObject: IDashboard) {
    return (dispatch: ThunkDispatch<{}, {}, any>) => {
        ...
        return dashboardervice().addDashboard(data)
            .then((result) => {
                if (result.status === 200) {
                    return dispatch(addDashboardSuccess(result.body));
                }
                return dispatch(addDashboardFailure({
                    text: "Status from server " + result.status
                }));
            })
            .catch((err) => {
...
export function deleteDashboard(delObject: IDashboard) {
    return (dispatch: ThunkDispatch<{}, {}, any>) => {
        ...
        return dashboardervice().deleteDashboard(delObject)
            .then((result) => {
                if (result.status === 200) {
                    return dispatch(deleteDashboardSuccess(id));
                }
                return dispatch(deleteDashboardFailure({
                    text: "Status from server " + result.status
                }));
            })
            .catch((err) => dispatch(deleteDashboardFailure({
